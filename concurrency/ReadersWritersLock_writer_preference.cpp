...
public:
  void acquire_read_lock() {
    ...
    reader_cv.wait(lock, [&]() {
      return anyActiveWriter == false && waitingWriters == 0;
    });
  ...
  void release_read_lock() {
    ...
    if (activeReaders == 0)
      writer_cv.notify_one(); // notify blocked writer if any
  }
  ...
  void acquire_write_lock() {
    ...
    writer_cv.wait(
        lock, [&]() { return activeReaders == 0 && anyActiveWriter == false; });
  ...
  void release_write_lock() {
    ...
    if (waitingWriters > 0) // writer preference
      writer_cv.notify_one();
    else
      reader_cv.notify_all(); // need to notify all readers
  }
};
...
Writer : 3 updating value to : 3
Writer : 4 updating value to : 4
Writer : 5 updating value to : 5
Reader : 2 reading value : 5
Reader : 3 reading value : 5
Reader : 4 reading value : 5
Reader : 5 reading value : 5
*/
